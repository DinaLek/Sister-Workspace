...
class PageBuilder:
    ...
    def _draw_service_block(self, service: dict):
        ...
        if options:
            ...
            for extra in service.get("price_lines", []):
                self._c.setFont("Assistant-SemiBold", 12)
                self._c.setFillColor(DARK)
                self._c.drawRightString(TEXT_RIGHT, self._y, rtl(extra))
                self._y -= 16
    ...
    def _estimate_service_height(service: dict) -> float:
        ...
        price_lines = len(service.get("price_lines", [])) * 16 if not service.get("options") else 0
        return base + bullets + options + price + price_lines + 24
